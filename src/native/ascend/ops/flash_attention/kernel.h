...
// already on the host and can be read directly without a D2H sync.
 ...
 public:
  ...
  void operator()(const Tensor query, const Tensor key, const Tensor value,
    ...
    if (!paged) {
      ...
      // K/V descriptors go into TensorList which takes ownership. They must be
      ...
      // `t_q` and `t_out` are owned by caches. Do not destroy them.
    ...
    // `t_query`, `t_output`, and `t_block_table` are owned by caches. Do not
    // destroy them.
