...
//
// Computes `residual_out = input + residual` and
// `out = rms_norm(residual_out, weight, eps)` in a single CANN launch.
// The fused API has higher host-side launch overhead (~200 us) compared to the
// decomposed `aclnnAdd` + `aclnnRmsNorm` path (~39 us), but may offer better
// NPU-side efficiency for large tensors where kernel fusion reduces memory
// traffic.
//
