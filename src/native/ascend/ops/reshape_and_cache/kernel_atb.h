...
class Operator<ReshapeAndCache, Device::Type::kAscend, 2>
 ...
 public:
  Operator(const Tensor key, const Tensor value, const Tensor key_cache,
    ...
    // Prepare aclnnCast descriptors for the int64 to int32 path.  Source
  ...
  void operator()(const Tensor key, const Tensor value, const Tensor key_cache,
    ...
    // `aclnnCast` to the pre-allocated int32 device buffer. This keeps the
    ...
    atb::VariantPack vp = buildVariantPack(
        const_cast<void*>(key.data()), const_cast<void*>(value.data()),
        const_cast<void*>(key_cache.data()),
        const_cast<void*>(value_cache.data()), slot32_ptr);
 ...
 private:
  ...
  atb::VariantPack buildVariantPack(void* key_data, void* value_data,
    ...
    atb::Tensor t_kv_k =
        ascend::ToAtbTensor(kv_shape_, acl_dt_, key_cache_data, cache_bytes);

    atb::Tensor t_kv_v =
        ascend::ToAtbTensor(kv_shape_, acl_dt_, value_cache_data, cache_bytes);
  ...
  // Cached aclnnCast descriptors (int64 `slot_mapping` to int32 buffer).
