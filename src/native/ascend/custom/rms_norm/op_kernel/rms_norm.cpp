 ...
 private:
  ...
  __aicore__ inline void Compute(int64_t row) {
    ...
    if constexpr (sizeof(T) == 4) {
      ...
      // Cast result from `fp32` to `fp16` / `bf16`.  `CAST_RINT` is
      // round-to-nearest-even and is defined for both `half` and `bfloat16_t`
      // destinations; `CAST_ROUND` is a `half`-specific alias.
