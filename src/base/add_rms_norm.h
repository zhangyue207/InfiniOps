...
// `out = RMSNorm(residual_out) * weight`. The 4-arg overload
// `(input, residual, weight, eps)` aliases `out = input` and
// `residual_out = residual` to match vLLM's inplace `fused_add_rms_norm`
// schema.
